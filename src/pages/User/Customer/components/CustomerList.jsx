...
export default function CustomerList() {
  ...
  // update status customer -> useMutation react query
  const { mutate: updateStatusCustomer } = useMutation({
    mutationFn: async (id, status) => {
      // update customer
      return await userService.updateStatusCustomerById(id, status);
    },
    onSuccess: () => {
      // notification
      notification.showSuccess("Update customer status success !");
    ...
    onError: () => {
      // notification
      notification.showError("Update customer status failed !");
    },
  });

  // get all customer -> react query
  const { data, isLoading } = useQuery({
    queryKey: ["customers", search, status, page, size],
    queryFn: async () => {
  ...
  // loading get all customer -> react query
  if (isLoading) {
    return (
      <>
  ...
  const headerSidebarDetail = (
    <div className="flex flex-column align-items-center gap-2">
      <span className="text-xl font-medium">Detail Customer</span>
    </div>
  );
    ...
    return (
      <div>
        <div className="flex flex-row gap-3">
          ...
          <Button
            onClick={() => {
              setVisibleDetail(true), setSelectedCustomer(rowData);
            }}
            icon="pi pi-user"
            text
            raised
      ...
      >
        <div className="mt-2">
          ...
          <div className="p-3 mb-4 shadow-3">
            ...
            <div className="flex flex-row card mb-3">
              <div className="flex flex-column">
                ...
                <span className="text-xs">{selectedCustomer?.username}</span>
              </div>
            </div>
          ...
          <div className="p-3 mb-4 shadow-3">
            ...
            <div className="flex flex-row card mb-3">
              <div className="flex flex-column">
                <span className="text-xs font-semibold">KTP Number</span>
                <span className="text-xs">{selectedCustomer?.ktpNumber}</span>
              </div>
            </div>
            ...
            <div className="flex flex-row card mb-3">
              <div className="flex flex-column">
                ...
                <span className="text-xs">{selectedCustomer?.phoneNumber}</span>
              </div>
            </div>
          ...
          <div className="p-3 shadow-3">
            ...
            {/* Bank Data */}
            {selectedCustomer?.bankAccounts ? (
              selectedCustomer.bankAccounts.map((item, index) => {
                return (
